...
public abstract class UsersControllerBase : ControllerBase
{
    ...
    [HttpPost("login")]
    [Authorize(Roles = "user")]
    public async Task<User> UserLogin([FromBody()] UserCreateInput userCreateInputDto)
    {
        return await _service.UserLogin(userCreateInputDto);
    }

    [HttpPost("register")]
    [Authorize(Roles = "user")]
    public async Task<User> UserRegistration([FromBody()] UserCreateInput userCreateInputDto)
    {
        return await _service.UserRegistration(userCreateInputDto);
    }
